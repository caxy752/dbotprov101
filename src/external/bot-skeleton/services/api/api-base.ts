...
class APIBase {
    ...
    async ensureCurrentAppId() {
        // App ID switching is disabled. Do not reconnect mid-session.
        if (hasAppIdChanged()) {
            console.log('🔌 [APP ID] App ID change detected but switching is disabled. Keeping current connection.');
            return;
        }
